...
static void __buffer_work(struct k_work *work)
{
	...
	static enum arducam_frame_mode f_mode;
	...
	if (drv_data->fifo_length == 0) {
		...
		if (f_length > vbuf->size) {
			f_mode = ARDUCAM_FRAME_FRAGMENTED;
		} else {
			f_mode = ARDUCAM_FRAME_COMPLETE;
		}

		vframe->flag = ARDUCAM_VIDEO_BUF_START;
		...
		if (drv_data->fifo_length == 0) {
			vframe->flag = ARDUCAM_VIDEO_BUF_DONE;
		} else {
			vframe->flag = ARDUCAM_VIDEO_BUF_MORE;
		}
	}

	vbuf->timestamp = f_timestamp;
	vframe->frame_length = f_length;
	vframe->mode = f_mode;
